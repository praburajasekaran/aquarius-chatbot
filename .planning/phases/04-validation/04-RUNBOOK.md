...
### §2.1 Local ngrok variant (used 2026-05-21)

Use this when validating the BPoint chat form against a local Next dev server instead of a Vercel preview.

- [ ] Start/confirm local Next on port 3000.
- [ ] Start ngrok: `ngrok http 3000`
- [ ] Copy the HTTPS forwarding origin, e.g. `https://<subdomain>.ngrok-free.dev`.
- [ ] Restart Next with the public URL injected:
  ```
  NEXT_PUBLIC_URL=https://<subdomain>.ngrok-free.dev \
  APP_URL=https://<subdomain>.ngrok-free.dev \
  BPOINT_REDIRECT_BASE_URL=https://<subdomain>.ngrok-free.dev \
  npm run dev -- --webpack
  ```
- [ ] Use `--webpack` if Turbopack panics on untracked GSD symlinks at repo root (`bin`, `contexts`, `references`, `templates`, `workflows`).
- [ ] Open the ngrok URL in a browser. Prefer incognito or disable the One Sec extension; otherwise React logs a harmless hydration mismatch on `<html one-sec-browser-extension-id="...">`.
- [ ] Drive the chat flow through contact details, urgency selection, and payment.
- [ ] Expected minimum local browser proof: `Payment completed successfully.` appears and the `Upload Supporting Documents` step renders.

**Caveats for ngrok-local runs**
- `tests/uat/setup.ts` currently rejects non-`.vercel.app` URLs, so the automated UAT smoke suite does not run unchanged against ngrok.
- Local email evidence depends on valid local `RESEND_API_KEY`; on 2026-05-21 local Resend returned `validation_error — API key is invalid`.
- The ngrok-local pass proves browser/payment continuity, not full cutover evidence. TEST-01 remains partial until receipt email, upload-token URL, logs, and smoke tests are captured.
...
### §3.1 2026-05-21 ngrok-local discoveries folded into code

During ngrok-local TEST-01 validation, the following defects surfaced and were fixed before the browser reached the upload step:

- Duplicate/concurrent checkout setup could create multiple AuthKeys for one session and then fail `/api/checkout/process` with `authkey_mismatch`. Fix: atomic `bpoint-authkey:{sessionId}` Redis claim plus existing AuthKey reuse.
- BPoint process could succeed, then `/api/checkout/confirm` could redirect to `?payment=failed&reason=system` because `retrieveTransaction(ResultKey)` returned `APIResponse.ResponseCode=118 "Invalid transaction number"`. Fix: store `bpoint-result:{ResultKey}` fallback from the successful process response and let confirm route use it.
- Payment could render before intake creation if the model called `initiatePayment` before `selectUrgency`. Fix: clearer PaymentCard not-ready UI plus stricter tool/prompt guard.
