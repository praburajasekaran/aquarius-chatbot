...
- Stripe dedup with `stripe-session:{sessionId}` NX + `pending` recovery ✅
- Upload token mint ✅
...
The Stripe webhook (`src/app/api/webhooks/stripe/route.ts`) already calls `handleIntakePaid()` ✅
...
- **SMS-05** — `sms-immediate:{sessionId}` Redis NX key is NOT implemented inside `handleIntakePaid()`. The top-level `stripe-session:` dedup already blocks retries in the common case, but the `pending` recovery path can re-enter the SMS dispatch. Adding the NX dedup inside the SMS dispatch block provides the defense-in-depth the requirement asks for.
...
- **TEST-02** — Integration test for Stripe webhook retry → exactly one SMS dispatched (tests SMS-05 dedup).
...
- EVENT-02 (Stripe webhook refactored) — webhook already delegates
...
| EVENT-02 | Stripe webhook refactored | ✅ Done | None |
...
| TEST-02 | Stripe retry integration test | ❌ Missing | Write integration test |
...
2. Stripe webhook retry → exactly one SMS send → TEST-02 will verify this
